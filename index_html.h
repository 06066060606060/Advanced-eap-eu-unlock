...
<div class="wrap">
  ...
  <section class="quick">
    ...
    <div class="qrow"><div class="ql"><div class="ico">R</div><div><div class="qt">Restore TLSSC</div><div class="qs">For banned car only</div></div></div><label class="toggle"><input id="tlsscRestoreToggle" type="checkbox"><span class="track"></span></label></div>
  </section>
...
$('tlsscRestoreToggle').onchange=async e=>{try{await post(e.target.checked?'/api/summon/tlssc-restore-enable':'/api/summon/tlssc-restore-disable');await fetchSum();toast(e.target.checked?'Restore TLSSC ON':'Restore TLSSC OFF')}catch(x){e.target.checked=!e.target.checked;toast('Restore TLSSC update failed')}};
