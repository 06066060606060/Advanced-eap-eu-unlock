...
<body>
<div class="wrap">
  <div class="top"><div class="logo">MODEL YL UNLOCK</div><div class="live" id="conn">Connecting</div></div>
  ...
  <section class="quick">
    ...
    <div class="qrow"><div class="ql"><div class="ico">↔</div><div><div class="qt">Auto Blinker</div><div class="qs" id="blinkSub">NOA only</div></div></div><label class="toggle"><input id="blinkToggle" type="checkbox"><span class="track"></span></label></div>
  ...
  <div class="canline">
    ...
    <section class="can"><div class="canTop"><div class="canName">CAN B</div><div class="badge" id="canBBadge">—</div></div><div class="canVal">VH</div><div class="canSub">TWAI</div></section>
  </div>
  ...
  <div class="bottom">MODEL YL UNLOCK</div>
</div>
<div class="toast" id="toast"></div>
<script>
const $=id=>document.getElementById(id);let nagCfg=null,otaUploading=false,lastOk=0,lastBlink=null,lastNag=null,lastSum=null,lastSys=null,lastDas=null;
...
$('blinkApply').onclick=async()=>{let sec=parseFloat($('blinkDelay').value);if(!Number.isFinite(sec))sec=2.0;sec=Math.max(0,Math.min(30,sec));const v=Math.round(sec*1000);try{await post('/api/blinkA/delay?ms='+v);fetchBlink();toast('Delay '+(v/1000).toFixed(1)+' s')}catch(e){}};
...
loadNag();fetchNag();fetchBlink();fetchDas();fetchSum();fetchSys();setInterval(()=>{if(!otaUploading){fetchNag();fetchBlink()}},500);setInterval(()=>{if(!otaUploading){fetchDas();fetchSum()}},800);setInterval(()=>{if(!otaUploading)fetchSys()},3000);
</script>
</body></html>
)HTML";
